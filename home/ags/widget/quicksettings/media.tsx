...
export default () => <stack
  setup={self => print(self.shown)}
  visible={bind(mpris, "players").as(p => p.length > 0)}>
  {bind(mpris, "players").as(p => p.map((player,index) =>
    <box name={index.toString()} 
    css={`padding: 10px; background:@theme_bg_color;
    border-radius: 12px;`}>
      <box className={"image"}
      ...
      <box vertical hexpand>
        ...
        <slider className={"position"}
          ...
          value={bind(player, "position").as(pos => pos / player.length)} />
        <centerbox>
          <label css={`color:@theme_text_color`}
            label={bind(player, "position").as(pos => lengthStr(pos))} />
          <box>
            <button onClick={() => player.previous()} visible={player.canGoPrevious}>
            ...
            <button onClick={() => player.next()} visible={player.canGoNext} >
              <icon icon={"media-skip-forward-symbolic"} />
            </button>
          </box>
          <label label={lengthStr(player.length)} css={`color:@theme_text_color`} />
        </centerbox>
      </box>
    </box >
  ))}
