...
const backlight_popup = () =>
  default_popup(
    MaterialIcon("brightness_1").hook(brightness, (self) => {
      self.label = getBrightnessIcon(brightness.screen_value);
    }),
    brightness.bind("screen_value").as((n) => n * 100),
    (self) => {
      brightness.screen_value = self.value / 100;
    },
    (self) => {
